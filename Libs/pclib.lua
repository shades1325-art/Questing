...
function pc.getPokemonTotalStatFromPC(boxId, pokemonIndex)
	local pokemonName = getPokemonNameFromPC(boxId, pokemonIndex)
	return luaPokemonData[pokemonName]["TotalStats"]
end

function pc.getBestPokemonIdFromCurrentBox()
	local bestPokemonInBox = 1 --asume first is highest
...
function pc.getBestPokemonIdFromCurrentBoxFromRegion(region)
	local bestPokemonInBox = nil --asume there's no pokemon from the current region in the box
