...
local name        = 'SandgemTownQuest'
local description = 'Sandgem Town to Jubilife City'
local level       = 0
...
local SandgemTownQuest = Quest:new()

function SandgemTownQuest:new()
	local o = Quest.new(SandgemTownQuest, name, description, level, dialogs)
	o.pokemonId = 1
	return o
end

function SandgemTownQuest:isDoable()
...
function SandgemTownQuest:isDone()
	return getMapName() == "Jubilife City"
end

function SandgemTownQuest:SandgemTown()
    ...
    elseif dialogs.RowanLabCheck.state then
		sys.debug("quest", "Going to tell grandma")
		return moveToCell(0, 12)
    elseif dialogs.SandgemBarry.state then
        sys.debug("quest", "Going to Rowan Lab")
...
function SandgemTownQuest:RowanLab()
...
function SandgemTownQuest:PokecenterSandgemTown()
...
function SandgemTownQuest:TwinleafTownPlayerHouse()
...
function SandgemTownQuest:TwinleafTown()
	if dialogs.RowanLabCheck.state then
		sys.debug("quest", "Going to tell grandma")
...
function SandgemTownQuest:TwinleafTownRivalHouse()
...
function SandgemTownQuest:TwinleafTownRivalHouse2F()
...
function SandgemTownQuest:Route201()
	if dialogs.RowanLabCheck.state then
		sys.debug("quest", "Going to tell grandma")
