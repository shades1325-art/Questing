...
local P7ThunderBadgeQuest = Quest:new()

function P7ThunderBadgeQuest:new()
	o = Quest.new(P7ThunderBadgeQuest, name, description, level, dialogs)
	o.pokemonId = 1
...
function P7ThunderBadgeQuest:isDoable()
...
function P7ThunderBadgeQuest:isDone()
	if getMapName() == "Vermilion City 2" or getMapName() == "SSAnne 1F" or getMapName() == "Route 11" or hasItem("Thunder Badge") then
		return true
	else
		return false
	end
...
function P7ThunderBadgeQuest:UndergroundHouse1()
	sys.debug("quest", "Going to Vermilion City.")
	return moveToCell(9,3)
end

function P7ThunderBadgeQuest:Underground2()
	sys.debug("quest", "Going to Vermilion City.")
	return moveToCell(3,43)
end

function P7ThunderBadgeQuest:UndergroundHouse2()
	sys.debug("quest", "Going to Vermilion City.")
	return moveToCell(5,10)
end

function P7ThunderBadgeQuest:Route6()
	...
	if not dialogs.psychicWadePart2.state then
		self:debug("quest", "Going to talk to Psychic Wade.")
...
function P7ThunderBadgeQuest:VermilionCityGraveyard()
...
function P7ThunderBadgeQuest:PokecenterVermilion()
...
function P7ThunderBadgeQuest:PokecenterCerulean()
	self:pokecenter("Cerulean City")
end

function P7ThunderBadgeQuest:VermilionPokemart()
	self:pokemart("Vermilion City")
end

function P7ThunderBadgeQuest:CeruleanCity()
	sys.debug("quest", "Going back to Route 5 after blackout.")
	return moveToCell(16, 50)
end


function P7ThunderBadgeQuest:VermilionCity()
...
function P7ThunderBadgeQuest:FishermanHouseVermilion()
	if not hasItem("Old Rod") then
		sys.debug("quest", "Going to get Old Rod.")
...
function P7ThunderBadgeQuest:puzzleBinPosition(binId)
	local xCount = 5
	local yCount = 3
	local xPosition = 2
	local yPosition = 17
...
function P7ThunderBadgeQuest:solvePuzzle()
...
function P7ThunderBadgeQuest:VermilionGym()
