...
local P25VolcanoBadgeQuest = Quest:new()

function P25VolcanoBadgeQuest:new()
	local o = Quest.new(P25VolcanoBadgeQuest, name, description, level)
	o.checkedForBestPokemon = false
	o.relogged = false
	return o
end

function P25VolcanoBadgeQuest:isDoable()
...
function P25VolcanoBadgeQuest:isDone()
...
function P25VolcanoBadgeQuest:isTrainingOver()
...
function P25VolcanoBadgeQuest:farmForBikeNeeded()
...
function P25VolcanoBadgeQuest:CinnabarIsland()
...
function P25VolcanoBadgeQuest:PokecenterCinnabar()
	if not game.isTeamFullyHealed() then
		sys.debug("quest", "Going to heal Pokemon.")
		return talkToNpcOnCell(5, 15)
	else
		self.registeredPokecenter = "Pokecenter Cinnabar"
		return moveToCell(8, 22)
	end
end

function P25VolcanoBadgeQuest:CinnabarPokemart()
	self:pokemart()
end

function P25VolcanoBadgeQuest:Route20()
...
function P25VolcanoBadgeQuest:Seafoam1F()
	if not self:isTrainingOver() then
		sys.debug("quest", "Going to level Pokemon until Level " .. self.level .. ".")
...
function P25VolcanoBadgeQuest:SeafoamB1F()
	if not self:isTrainingOver() then
		sys.debug("quest", "Going to level Pokemon until Level " .. self.level .. ".")
...
function P25VolcanoBadgeQuest:SeafoamB2F()
	if not self:isTrainingOver() then
		sys.debug("quest", "Going to level Pokemon until Level " .. self.level .. ".")
...
function P25VolcanoBadgeQuest:SeafoamB3F()
	if not self:isTrainingOver() then
		sys.debug("quest", "Going to level Pokemon until Level " .. self.level .. ".")
...
function P25VolcanoBadgeQuest:SeafoamB4F()
...
function P25VolcanoBadgeQuest:CinnabarGym()
...
function P25VolcanoBadgeQuest:CinnabarGymB1F()
	if not hasItem("Volcano Badge") then
