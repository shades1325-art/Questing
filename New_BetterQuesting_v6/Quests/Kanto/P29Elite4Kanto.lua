...
local P29Elite4Kanto = Quest:new()

function P29Elite4Kanto:new()
	return Quest.new(P29Elite4Kanto, name, description, level, dialogs)
end

function P29Elite4Kanto:isDoable()
	return self:hasMap()
end

function P29Elite4Kanto:isDone()
...
function P29Elite4Kanto:useReviveItems() --Return false if team don't need heal
...
function P29Elite4Kanto:EliteFourLoreleiRoom()
...
function P29Elite4Kanto:EliteFourBrunoRoom()
	if self:useReviveItems() ~= false then
		return
	elseif not dialogs.leaderDone.state then
		sys.debug("quest", "Going to fight Bruno.")
...
function P29Elite4Kanto:EliteFourAgathaRoom()
	if self:useReviveItems() ~= false then
		return
	elseif not dialogs.leaderDone.state then
		sys.debug("quest", "Going to fight Agatha.")
...
function P29Elite4Kanto:EliteFourLanceRoom()
	if self:useReviveItems() ~= false then
		return
	elseif not dialogs.leaderDone.state then
		sys.debug("quest", "Going to fight Lance.")
...
function P29Elite4Kanto:EliteFourChampionRoom()
	if self:useReviveItems() ~= false then
		return
	elseif not dialogs.leaderDone.state then
		sys.debug("quest", "Going to fight Gary.")
