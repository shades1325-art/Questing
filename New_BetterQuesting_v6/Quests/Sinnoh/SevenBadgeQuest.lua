...
function SevenBadgeQuest:isDoable()
	if self:hasMap() and hasItem("Relic Badge") and not hasItem("Icicle Badge") then
		return true
	end
	return false
end
...
function SevenBadgeQuest:isDone()
	if hasItem("Icicle Badge") then --fix blackout
		return true
	end
end
...
function SevenBadgeQuest:Route211()
    if self.dialogs.LakeAcuity.state then
        ...
        return moveToCell(0, 26)
    end
end
function SevenBadgeQuest:MtCoronetCenter()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Lake Acuity.")
        return moveToCell(12, 5)
    else
        sys.debug("quest", "Going to Eterna City.")
        return moveToCell(2, 28)
    end
end
function SevenBadgeQuest:MtCoronetB1F()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Lake Acuity.")
...
function SevenBadgeQuest:MtCoronetNorth()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Lake Acuity.")
...
function SevenBadgeQuest:Route216()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Lake Acuity.")
...
function SevenBadgeQuest:Route217()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Lake Acuity.")
        return moveToCell(33, 0)
    else
        sys.debug("quest", "Going to Eterna City.")
        return moveToCell(19, 114)
    end
end
function SevenBadgeQuest:AcuityLakefront()
    if self.dialogs.LakeAcuity.state then
        sys.debug("quest", "Going to Snowpoint City.")
        return moveToCell(70, 36)
    else
        sys.debug("quest", "Going to Eterna City.")
        return moveToCell(22, 49)
    end
end
function SevenBadgeQuest:SnowpointCity()
	if not game.isTeamFullyHealed() then
        self.ex = 0
		return moveToCell(30, 37)
    elseif not hasItem("Icicle Badge") then
        self.ex = 0
		return moveToCell(18, 29)
	end
end
function SevenBadgeQuest:PokecenterSnowpointCity()
	if not game.isTeamFullyHealed() then
		return talkToNpcOnCell(8, 7)
	else
...
function SevenBadgeQuest:SnowpointGym()
	if not hasItem("Icicle Badge") then
        if self.ex == 0 then
            self.ex = 0.1
        ...
        elseif self.ex == 0.1 then
            self.ex = 0.2
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return talkToNpcOnCell(6, 23)
        elseif self.ex == 0.2 then
            self.ex = 0.3
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return moveToCell(7, 8)
        ...
        elseif self.ex == 0.4 then
            self.ex = 0.5
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return moveToCell(16, 9)
        elseif self.ex == 0.5 then
            self.ex = 0.6
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return moveToCell(13, 9)
        elseif self.ex == 0.6 then
            self.ex = 0.7
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return moveToCell(13, 5)
        elseif not hasItem(13, 2) then
            self.ex = 0.8
            sys.debug("quest", "Current| ex = " .. tostring(self.ex))
            return talkToNpcOnCell(13, 2)
        end
	else
		return moveToCell(13, 19)
	end
end
...
function SevenBadgeQuest:PokecenterSandgemTown()
	if not game.isTeamFullyHealed() then
		return talkToNpcOnCell(8, 7)
	else
